<section class="flex h-full w-full flex-1 flex-col gap-6">
    <div class="flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl">{{ __('Guard Documents') }}</flux:heading>
            <flux:text>{{ __('Private documents attached to guard records.') }}</flux:text>
        </div>
    ...
    <flux:modal wire:model.self="showUploadModal" class="md:w-[34rem]">
        <form wire:submit="save" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Upload Guard Document') }}</flux:heading>
