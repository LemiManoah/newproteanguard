<section class="flex h-full w-full flex-1 flex-col gap-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <flux:text>{{ __('Manage the categories used when recording expenses.') }}</flux:text>
        </div>
        ...
        <flux:table.rows>
            @forelse ($categories as $category)
                <flux:table.row :key="$category->id">
                    ...
                    <flux:table.cell>{{ number_format((int) $category->expenses_count) }}</flux:table.cell>
                    <flux:table.cell>{{ $category->created_at?->toFormattedDateString() }}</flux:table.cell>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="5">
                        <div class="py-8 text-center text-sm text-zinc-500">{{ __('No expense categories found.') }}</div>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <flux:modal wire:model.self="showFormModal" class="md:w-[30rem]">
        <form wire:submit="save" class="space-y-5">
            <flux:heading size="lg">{{ $editingId ? __('Edit Category') : __('New Expense Category') }}</flux:heading>
